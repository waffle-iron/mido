...
ws_blast = function (options) {
    ...
    this.init = function (aCont) {
        ...
        for (var index = 0; index < columns * rows; index++) {
            ...
            var left0 = Math.round(options.width * i / columns);
            var top0 = Math.round(options.height * j / rows);
            var left1 = Math.round(options.width * (i + 1) / columns);
            var top1 = Math.round(options.height * (j + 1) / rows);
            $([partsIn[index] = document.createElement("div"), partsOut[index] = document.createElement("div")]).css({
                position: "absolute",
                width: left1 - left0,
                height: top1 - top0,
    ...
    function setPos(parts, random, animate) {
        var pWidth = options.width / columns;
        var pHeight = options.width / rows;
        var wpos = {
        ...
        $(parts).each(function (index) {
            if (random) {
                var left0 = distance * options.width * (2 * Math.random() - 1) + options.width / 2;
                var top0 = distance * options.height * (2 * Math.random() - 1) + options.height / 2;
                var gpos = $partCont.offset();
                gpos.left += left0;
                gpos.top += top0;
                if (gpos.left < wpos.left) {
                    left0 -= gpos.left + wpos.left;
                }
                if (gpos.top < wpos.top) {
                    top0 -= gpos.top + wpos.top;
                }
                if (gpos.left > wpos.left + wpos.width - pWidth) {
                    left0 -= gpos.left - (wpos.left + wpos.width) + pWidth;
                }
                if (gpos.top > wpos.top + wpos.height - pHeight) {
                    top0 -= gpos.top - (wpos.top + wpos.height) + pHeight;
                }
            } else {
                var left0 = Math.round(options.width * (index % columns) / columns);
                var top0 = Math.round(options.height * Math.floor(index / columns) / rows);
            }
            if (animate) {
                $(this).animate({
                    left: left0,
    ...
    this.go = function (new_index) {
        ...
        $(partsOut).css({
            opacity: 1,
            'background-image': "url(\"" + $(Images[curIdx]).attr("src") + "\")"
        });
        $(partsIn).css({
            opacity: 0,
            'background-image': "url(\"" + $(Images[new_index]).attr("src") + "\")"
        });
        setPos(partsIn, false, true);
        $(partsIn).animate({
            opacity: 1
        }, {
            queue: false,
